...
def product_quantity_ordered():
    total_orders = Product.objects.annotate(
        total_ordered_quantity=Sum('orderproduct__quantity')
    ).filter(total_ordered_quantity__gte=1).order_by('-total_ordered_quantity')

    return '\n'.join(f"Quantity ordered of {p.name}: {p.total_ordered_quantity}" for p in total_orders)


def ordered_products_per_customer():
    ordered_products = Order.objects.prefetch_related("orderproduct_set__product__category").order_by('id')
