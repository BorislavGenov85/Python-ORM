...
def find_books_by_genre_and_language(genre, language):
    sorted_books = Book.objects.filter(genre=genre, language=language)
...
def delete_review_by_id(num):
    current_review = Review.objects.get(id=num)
    Review.objects.get(id=num).delete()
    return f"Review by {current_review.reviewer_name} was deleted"
...
def filter_authors_by_nationalities(nationality):
    ...
    result = [author.biography if author.biography else f"{author.first_name} {author.last_name}"
...
def filter_authors_by_birth_year(first_year, second_year):
    ...
    result = [f"{author.birth_date}: {author.first_name} {author.last_name}" for author in filtered_authors]
